...
class PlanDeEstudio
{
     ...
     * @return bool
     */
    private function _get_inner_query(int $id) : array {
        $ban = false;
        $sqlquery = "SELECT usuario.nombre, usuario.apellidos, grupo.clave, grupo.turno, grupo.cuatrimestre, materia.nombre as materia, materia_en_grupo.modificador_horas as modif from usuario INNER JOIN plan_de_estudio ON (plan_de_estudio.carrera =?) INNER JOIN carga_academica ON ( carga_academica.plan_estudios = plan_de_estudio.id ) INNER JOIN grupo ON( grupo.carga_academica = carga_academica.id ) INNER JOIN materia_en_grupo ON( materia_en_grupo.grupo = grupo.id ) INNER JOIN materia ON ( materia.id = materia_en_grupo.materia ) INNER JOIN profesor on(profesor.id=materia_en_grupo.profesor) WHERE usuario.id=profesor.usuario ORDER By grupo.cuatrimestre; ";
        try {
            $res = $this->SqlOp->exec($sqlquery, "i", [$id]);
            if (count($res) != 1) {
                $t=0;
                foreach ($res as $r) {
                    $this->_asignar_inner($res[$t]["nombre"], $res[$t]["apellidos"], $res[$t]["clave"], $res[$t]["turno"], $res[$t]["cuatrimestre"], $res[$t]["materia"], $res[$t]["modif"]);
                    $t++;
                }
                $ban = true;
            }
