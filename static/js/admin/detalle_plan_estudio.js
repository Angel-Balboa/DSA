...
var submit_actualizar_datos_plan = function () {
    $("#frmDatosPlanEstudio").submit(function (e) {
        ...
        dataSubmit["clv_plan"] = clv_plan;

        // verificamos si la clave anterior es igual a la nueva
        if (dataSubmit.clv_plan === dataSubmit.nueva_clv_plan.trim()) {
            delete dataSubmit["nueva_clv_plan"]; // si es así, evitamos el envío de la nueva clave
        } else {
            ban_reload = true;
            var url = window.location.href.split('?')[0] + '?clv_plan=' + dataSubmit.nueva_clv_plan.trim();
        }

        var result = post_data("../../api/requests/admin/updates/plan_de_estudio/actualiza_datos.php", dataSubmit, true);
...
var submit_nueva_carga_academica = function () {
    $("#frmAgregarCargaAcademica").submit(function (e){
        ...
        var result = post_data("../../api/requests/admin/creates/carga_academica/crea_nueva_carga_academica.php", dataSubmit, true);

        if (result.done) {
            rep("Éxito", "Se ha agregado la carga académica con éxito", 's');
...
var submit_nueva_materia = function () {
    $("#frmAgregarMateria").submit(function (e) {
        ...
        var result = post_data("../../api/requests/admin/creates/materia/crea_materia.php", dataSubmit, true);
...
var submit_edit_carga = function () {
    $("#frmActualizarDatosCarga").submit(function(e) {
        ...
        var result = post_data("../../api/requests/admin/updates/carga_academica/actualiza_datos.php", dataSubmit, true);
...
var submit_actualizar_materia = function () {
    $("#frmActualizarDatosMateria").submit(function (e) {
        ...
        if ($clave_materia !== null) {
            ...
            var result = post_data("../../api/requests/admin/updates/materia/actualiza_datos.php", dataSubmit, true);
