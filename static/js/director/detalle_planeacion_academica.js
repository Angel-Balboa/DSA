var initd = function () {
    $id_planeacion = getUrlParameter("id_planeacion");

    if ($id_planeacion !== null) {
        $("#btnAceptarPlaneacion").hide();
        obten_datos_planeacion($id_planeacion);
        aceptar_planeacion();
    } else {
        rep("Error", "Se debe proporcionar el Id de la planeación Académica", 'e');
    }
};

var aceptar_planeacion = function () {
    $("#btnAceptarPlaneacion").click(function () {
...
var obten_datos_planeacion = function (id_planeacion) {
    ...
    var result = get_data("../../api/requests/common/getters/planeacion_academica/obten_datos_completos_planeacion.php?id_planeacion=" + id_planeacion, true);
    ...
    if (result.done) {
        ...
        if (result.data.planeacion.estado === "finalizada") {
            var $btnAceptarPlaneacion = $("#btnAceptarPlaneacion");
            $btnAceptarPlaneacion.data("id-planeacion", id_planeacion);
            $btnAceptarPlaneacion.show();
        }
    }
};
